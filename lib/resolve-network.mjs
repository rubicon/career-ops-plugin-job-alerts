...
//
// What survives this gate and should not be mistaken for identity on its own: a
// tenant token SHORTER than the company name still relates to it, so a board named
// `northstar` passes for "Northstar Technologies" even when it belongs to Northstar
// Labs. Gate 3 is what separates those, which is why corroboration is a second
// independent signal here and not a formality.
...
//
// It removes furniture only. The core is still held to MIN_SLUG_OVERLAP, and still
// has to relate to the company's WHOLE name -- so stripping cannot readmit the
// first-word class: `careers-nimbusai` reduces to `nimbusai`, which relates to
// "Nimbus Data" no better than it did before.
//
