...
// long and that segment IS a job word, since a bare domain gets here only via
...
function looksLikeEmployerPosting(parsed, leadTokens) {
  ...
  // The final segment has to name the posting, and a job word in it names a
  // CATEGORY of postings ("vp-marketing-jobs") rather than one. A job word is never
  // a role word: pageTitleTokens already drops it from the result title, so a page
  // titled "VP Marketing Jobs" scores a perfect match against a "VP Marketing" lead
  // -- while counting the same word here as one of the role's own words let the
  // category page clear this gate by carrying the very word that marks it as a
  // category. The two normalizations now agree about what a job word is worth.
  const lastTokens = titleTokens(last);
  if (lastTokens.some((token) => JOB_PATH_WORDS.has(token))) return false;
