...
//           vendor, so here EQUALITY against the company name is required where (a)
//           and (b) accept a containment relation against it. All three read the
//           WHOLE name: what separates them is equality versus containment, never
//           the name versus a truncation of it (#24). The
...
//      board. The relation is against the company's WHOLE name and never a
//      truncation of it -- see slugRelated for why the vendor namespace does not buy
//      that. (c) does not use this gate; its 1c equality test is stricter.
...
//   - A board slug or tenant label that relates only to the company's FIRST WORD:
//     "nimbusai" for "Nimbus Data". The vendor host makes it somebody's real board,
//     which is why containment is allowed here at all, but it does not say whose.
...
async function resolveViaTavily(ctx, lead, breaker) {
  ...
  // The acceptance gates read the full-name forms. candidateSlugs, which also offers
  // the bare first word, stays where a wrong guess costs a 404: the Tier-1 probe.
  const companySlugs = fullNameSlugs(company);
...
//     site, so this gate reads fullNameSlugs, as gate 2 does.
...
// fullNameSlugs returns the slug forms that ARE this company's name: the joined and
// hyphen-joined forms of every significant word, in that order. No truncation of any
// kind. Every gate that reads a slug out of a URL and asks "does this identify the
// employer" reads this set -- gate 2's tenant relation and gate 1c's domain equality
// alike. `candidateSlugs`, which also offers the bare first word, is the Tier-1 PROBE
// set and belongs nowhere else (#24).
function fullNameSlugs(company) {
...
// employerDomainSlugs is fullNameSlugs plus one refusal: nothing for a one-word
// company.
//
...
function employerDomainSlugs(company) {
  if (companyWords(company).length < 2) return [];
  return fullNameSlugs(company);
}
...
// slugRelated decides whether the tenant token in a posting URL -- a shared board's
// slug in the path, or a per-employer tenant's host label -- plausibly belongs to
// this company.
//
// EQUALITY IS NOT REQUIRED, and this is the honest asymmetry with gate 1c above.
// Tier 1 already probes the exact full-name forms, so a slug equal to one of them is
// a board Tier 1 would have found; what reaches here is the miss class, a board token
// that is a VARIANT of the display name. That variant is namespaced inside a vendor
// host no outsider can register, where a bespoke domain label is global and anyone
// may hold the neighbouring registration. So a containment relation in either
// direction, with a length floor on the shorter side, is the evidence:
//
//   token INSIDE the name   "acmetech" in "acmetechnologies" -- an abbreviation.
//   name INSIDE the token   "acmetechnologies" in "acmetechnologies-careers" -- the
//                           whole name plus a suffix.
//
// WHAT THE VENDOR NAMESPACE DOES NOT BUY is a relation to a TRUNCATION of the name.
// The vendor host proves the URL is SOMEBODY's real board; it says nothing about
// whose, and that is the whole of what this gate has to decide. A token that merely
// extends the company's first word -- "nimbusai" for "Nimbus Data" -- is another
// employer whose name happens to begin the same way, and no string shape separates it
// from this employer's abbreviated board. Reading `candidateSlugs` here admitted
// exactly that class and nothing else: the first word is a prefix of the joined form,
// so it is redundant in the token-inside-the-name direction, while in the other
// direction it was the sole admitting entry (#24).
//
// The remaining loss is a board slug that extends the first word and IS this
// employer's -- "nimbusai" when Nimbus Data really does board there. That keeps the
// honest search fallback, which is the trade this tier is built on: a wrong canonical
// URL is worse than none, because it looks correct.
//
// WHAT THIS GATE STILL DOES NOT DECIDE, stated so it is not mistaken for identity:
// the token-inside-the-name direction accepts a token that stops at a WORD boundary
// of the company name, which is a different, shorter company. `northstar` passes for
// "Northstar Technologies" though it may be Northstar Labs' board, and `nimbus`
// passes for "Nimbus Data" though it may be Nimbus AI's. That is the same family as
// the truncation #24 removed, seen from the other direction, and closing it is a
// separate trade: on a shared board a first-word slug is what Tier 1 already probes
// and VERIFIES by fetching the board, while on a tenant platform there is no probe
// and rejecting it would lose real postings. Not decided here; filed as
// rubicon/career-ops-plugin-job-alerts#29 with the reproduction, which predates this
// change and is unaffected by it. Gate 3 is the backstop meanwhile, which is why
// corroboration is a second independent signal and not a formality.
function slugRelated(tenantSlug, companySlugs) {
  const candidates = companySlugs
    .map((slug) => slug.replace(/[^a-z0-9]/g, ''))
    .filter((candidate) => candidate.length >= MIN_SLUG_OVERLAP);
  if (candidates.length === 0) return false;
  for (const board of tenantIdentityForms(tenantSlug)) {
...
// tenantIdentityForms yields the forms of a tenant token whose relation to the
// company name is worth reading: the token as written, and -- when careers-style
// words bracket it -- the identity core alone.
//
// The bracketing words are the VENDOR's or the SITE's furniture, not the employer's
// identity. iCIMS's documented tenant host is `careers-{tenant}.icims.com`, so
// `careers-acmetech` is Acme Technologies' tenant with a vendor prefix on it, and a
// board slug is as readily written `{tenant}-careers`. This is the same set of labels
// registrableIdentityLabel sets aside on a bespoke host, applied inside one label
// rather than across dot-separated ones.
//
// It removes furniture only, and the result is held to the same relation as any other
// tenant token: the core still has to clear MIN_SLUG_OVERLAP and still has to relate
// to the company's WHOLE name. It cannot readmit what gate 2 now rejects in the
// name-inside-the-token direction -- `careers-nimbusai` reduces to `nimbusai`, which
// relates to "Nimbus Data" no better than it did before.
//
// What it DOES inherit is the standing weakness of the other direction, said here
// rather than left to be discovered: stripping shortens the token, and a shorter token
// is easier to find INSIDE the company name. `careers-nimbus` reduces to `nimbus`,
// which sits in "nimbusdata" and passes -- exactly as the bare board slug `nimbus`
// already does, on this branch and before it. That is a property of accepting an
// abbreviation at all, not of the strip: see the residual named at the end of
// slugRelated, and rubicon/career-ops-plugin-job-alerts#29.
//
// Stripping is allowed to empty the core, and needs no guard against it. A tenant
// made of nothing but furniture reduces to the empty string, which the length floor
// drops, and the written form is read either way -- so the emptied core can neither
// lose a match nor add one. A `core.length > 1` bound here would read as a rule and
// hold nothing: no input can tell it from this one.
function tenantIdentityForms(tenantSlug) {
  const tokens = String(tenantSlug ?? '')
    .toLowerCase()
    .split(/[^a-z0-9]+/)
    .filter(Boolean);
  let core = tokens;
  while (core.length > 0 && CAREERS_SUBDOMAINS.has(core[0])) core = core.slice(1);
  while (core.length > 0 && CAREERS_SUBDOMAINS.has(core[core.length - 1])) {
    core = core.slice(0, -1);
  }
  const forms = [tokens.join(''), core.join('')];
  return forms.filter((form, i) => form.length >= MIN_SLUG_OVERLAP && forms.indexOf(form) === i);
}
...
// 404 from a board API. It is safe in no acceptance gate, whether the gate reads a
// domain label or a board slug on a trusted vendor host -- there a wrong guess is
// emitted to the reader as this employer's posting. Those read fullNameSlugs.
