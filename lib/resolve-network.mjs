...
import { canonicalHostFamily } from './resolve-canonical.mjs';
...
// Path segments that say a URL is about jobs, used when the host itself does not.
const JOB_PATH_WORDS = new Set([
  'careers',
  'career',
  'jobs',
  'job',
...
// ...except a bare year, which is an archive or a listing by intake year
// ("/jobs/2026"), not a posting. It clears the digit floor on its own, so it has to
// be excluded by name.
const YEAR_SEGMENT_RE = /^(19|20)\d{2}$/;
...
//        c. EMPLOYER DOMAIN (careers.acme.com). No vendor suffix exists, so the
//           evidence is the domain's own identity label matching a slug derived
//           from the company name -- which comes from the lead, not from the result
//           being judged. A domain label is global rather than namespaced inside a
//           trusted vendor, so here EQUALITY is required where (a) and (b) accept a
//           containment relation, the host must be a shape this module recognizes
//           as a registrable domain, and gates 3 and 4 are both raised (below).
//
...
//   - An employer domain whose label is not exactly a company slug: acmetech.com or
//     trynova.io for "Acme Technologies" / "Nova". Nothing here establishes that
//     such a domain belongs to that employer, and containment on a GLOBAL namespace
//     is how a confident, wrong employer gets emitted.
...
async function resolveViaTavily(ctx, lead, breaker) {
  ...
  return bestSearchResult(title, company, companySlugs, companyTokens, results);
}
...
function bestSearchResult(leadTitle, company, companySlugs, companyTokens, results) {
  ...
  for (const result of results) {
    ...
    employer.set(url, score);
  }

  const hostedWinner = uniqueTopScorer(hosted);
  if (hostedWinner) return { url: hostedWinner, employerHosted: false };
  const employerWinner = uniqueTopScorer(employer);
  if (employerWinner) return { url: employerWinner, employerHosted: true };
  return null;
}

// uniqueTopScorer returns the single highest-scoring url, or null when the map is
// empty or two urls tie at the top. A tie is ambiguity, and an arbitrary pick among
// equally good matches is a guess, so it falls back instead.
function uniqueTopScorer(byUrl) {
  let best = null;
  let bestScore = 0;
  let tied = false;
...
//   - that identity label EQUALS one of the slugs derived from the company name.
//     A board slug only needs a containment relation because it is namespaced
//     inside a vendor whose host is already trusted; a domain label is global, so
//     acmetech.com and acmetechnologies.com may be entirely different owners and
//     containment would be a guess dressed as evidence.
function employerDomainOwns(parsed, companySlugs) {
  const label = registrableIdentityLabel(parsed.hostname);
  if (label === null) return false;
  return companySlugs.some((slug) => slug.toLowerCase() === label);
}
...
function looksLikeEmployerPosting(parsed, leadTokens) {
  ...
  const jobWordInPath = segments.some((segment) => JOB_PATH_WORDS.has(segment.toLowerCase()));
  if (!careersHost && !jobWordInPath) return false;
  // On a bare domain the job word is itself a path segment, so a posting needs one
  // more; on a careers host a single role segment is already the posting.
  if (!careersHost && segments.length < 2) return false;

  const last = segments[segments.length - 1];
  if (isPostingIdSegment(last)) return true;
  const lastTokens = new Set(titleTokens(last));
  let shared = 0;
  for (const token of new Set(leadTokens)) if (lastTokens.has(token)) shared++;
  return shared >= 2;
}

// hasPostingId decides whether a tenant-platform URL points at one posting. Both
// verified platforms put a numeric requisition or job id in the posting path and
// neither puts one in an index or a faceted-search path:
//   Workday  /en-US/{site}/job/{location}/{Role}_{JR-10423}
//   iCIMS    /jobs/{44120}/{role-slug}/job
function hasPostingId(parsed) {
  const segments = pathSegments(parsed);
...
function isPostingIdSegment(segment) {
  return POSTING_ID_RE.test(segment) && !YEAR_SEGMENT_RE.test(segment);
}
