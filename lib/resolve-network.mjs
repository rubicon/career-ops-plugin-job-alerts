...
// "Nimbus Data" no better than it did before. Stripping never empties the token: a
// tenant made of nothing but furniture keeps its written form and fails the floor.
function tenantIdentityForms(tenantSlug) {
  const tokens = String(tenantSlug ?? '')
    ...
    .filter(Boolean);
  let core = tokens;
  while (core.length > 1 && CAREERS_SUBDOMAINS.has(core[0])) core = core.slice(1);
  while (core.length > 1 && CAREERS_SUBDOMAINS.has(core[core.length - 1])) {
    core = core.slice(0, -1);
  }
