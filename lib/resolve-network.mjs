...
// Read from segmentTokens like every other vocabulary here, so `/careers-hub` and
// `/job-openings` count as readily as `/careers` does.
...
// The two vocabularies together. A path segment is read for jobs-or-not (above) and
// for listing-or-not (INDEX_PATH_WORDS, shared with the classifier) everywhere; the
// employer class's FINAL segment gets this stricter test, because there that one
// segment is the only thing identifying the posting and every one of these words
// names a CATEGORY of postings rather than one -- wherever in the segment it sits,
// not only where it brackets it. `/vp-marketing-jobs-2026` and
// `/vp-marketing-search-2026` are category pages with a year pinned on the end.
