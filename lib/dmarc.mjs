...
//
// A boundary that stamps no authserv-id is supported by passing `null`: the
// gate then reads the single Authentication-Results field that carries no
// authserv-id, and fails closed when there is more than one such field, since
// nothing distinguishes the boundary's own from an injected copy.
//
...
// authserv-id that boundary stamps, or null for one that stamps none; it is
// required, because there is no safe default. Absent, failing, unattributable
// and ambiguous results are all rejected.
export function passesDmarc(message, options) {
  ...
  const boundary =
    trusted === null
      ? fields.filter((f) => f.authservId === null)
      : fields.filter((f) => f.authservId === trusted);

  // With no authserv-id to match on, a single unnamed field is the only
  // unambiguous case: a second one could equally be the boundary's or the
  // sender's. Configure the boundary's authserv-id to resolve it.
  if (trusted === null && boundary.length !== 1) return false;
...
// trustedAuthservId normalizes the caller's declared boundary id. Anything that
// is neither a non-empty string nor an explicit null is a programming error --
// including the index that `messages.filter(passesDmarc)` would pass -- and
// throws rather than silently selecting a weaker rule.
function trustedAuthservId(options) {
  const value = options?.authservId;
  if (value === null) return null;
  if (typeof value === 'string' && value.trim() !== '') return value.trim().toLowerCase();
  throw new Error(
    'passesDmarc requires options.authservId: the authserv-id the receiving boundary ' +
      'stamps into Authentication-Results, or null for a boundary that stamps none.',
  );
}
