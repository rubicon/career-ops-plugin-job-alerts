...
// A bare number: a facet, an id, an offset. It names nothing on its own.
...
//
// A purely numeric token counts as neither -- it names nothing on its own, so it
// neither makes a segment a listing surface nor stops one from being one. That is
// what reads `page-2`, `offset-40` and `search-1250` as the facets they are, while
// `/jobs/44120/` stays the posting id it is: `44120` alone carries no index word.
//
...
export function looksLikeListingPath(segments) {
  return (segments ?? []).some((segment) => {
    ...
    return tokens.every(
      (token) =>
        INDEX_PATH_WORDS.has(token) || JOB_PATH_WORDS.has(token) || NUMERIC_TOKEN_RE.test(token),
    );
  });
}
