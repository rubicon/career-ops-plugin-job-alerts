...
export async function runIngest(ctx, deps = {}) {
  ...
  // source's knowledge (or the user's, via the authservId setting).
  ...
  if (unauthenticated > 0) {
    ...
    logLine(
      ...
      `job-alerts: skipped ${unauthenticated} of ${messages.length} message(s) with no ` +
        `dmarc=pass from ${authservId === null ? 'an unnamed receiving boundary' : authservId}. ` +
        'Set the authservId setting if your mail arrives through a different boundary.',
    );
  }
