...
// lib/headers.mjs encoding, exactly like the Gmail adapter.
...
//
// So the default here is null: read the single Authentication-Results field that
// carries no authserv-id, and fail closed if a second such field is present,
// because nothing then distinguishes the boundary's own from an injected copy.
// A tenant whose mail arrives through a boundary that does stamp an authserv-id
// (a hybrid or third-party gateway) should set the `authservId` setting to it,
// which is both stricter and unambiguous.
