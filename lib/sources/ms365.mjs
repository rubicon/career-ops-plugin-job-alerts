...
// collection into an object keyed by header name (preserving
// Authentication-Results), exactly like the Gmail adapter's headersToObject.
...
// headersToObject turns Graph's internetMessageHeaders [{name,value}] collection
// into a plain-object map keyed by header name, so the core DMARC gate can read
// message.headers as an object.
//
// Graph returns the full RFC 5322 header set, in which names legitimately
// repeat (Received, and often more than one Authentication-Results). Overwriting
// on a repeat would let a later hop that says nothing about DMARC shadow the one
// asserting dmarc=pass, so repeats are appended rather than replaced. Name
// casing follows the first occurrence and matching is case-insensitive, since
